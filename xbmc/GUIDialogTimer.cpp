...
bool CGUIDialogTimer::OnAction(const CAction &action)
{
  switch (action.wID)
  {
    ...
    case REMOTE_9:
    {
      ...
      m_typedString.Format("%s%d", m_typedString.c_str(), action.wID - REMOTE_0);
...
void CGUIDialogTimer::Render()
{
  ...
  {
    CStdString strTime = g_infoManager.LocalizeTime(dateTime, TIME_FORMAT_SHORT);
...
{
  CGUIDialogTimer *dialog = (CGUIDialogTimer *)m_gWindowManager.GetWindow(WINDOW_DIALOG_TIMER);
  if (!dialog) return -1;
