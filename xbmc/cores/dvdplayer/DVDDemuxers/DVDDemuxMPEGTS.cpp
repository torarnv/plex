/*
 *		Copyright (C) 2008 Ryan Walklin
 *		Based on XBMC FFmpeg Demuxer -  Copyright (C) 2005-2008 Team XBMC
 *		http://www.xbmc.org
 *		and Handbrake MPEG-TS Demuxer
...
void CDemuxStreamAudioMPEGTS::GetStreamInfo(std::string& strInfo)
{
	if(!m_stream) return;
	char temp[128];
...
void CDemuxStreamVideoMPEGTS::GetStreamInfo(std::string& strInfo)
{
	if(!m_stream) return;
	char temp[128];
	m_parent->m_dllAvCodec.avcodec_string(temp, 128, m_stream->codec, 0);
...
/*
void CDemuxStreamSubtitleMPEGTS::GetStreamInfo(std::string& strInfo)
{
	if(!m_stream) return;
	char temp[128];
	m_parent->m_dllAvCodec.avcodec_string(temp, 128, m_stream->codec, 0);
...
CDVDDemuxMPEGTS::CDVDDemuxMPEGTS() : CDVDDemux()
{
...
CDVDDemuxMPEGTS::~CDVDDemuxMPEGTS()
{
	Dispose();
	DeleteCriticalSection(&m_critSection);
}

bool CDVDDemuxMPEGTS::Open(CDVDInputStream* pInput)
{
	...
	if (!m_dllAvUtil.Load() || !m_dllAvCodec.Load() || !m_dllAvFormat.Load())  {
		CLog::Log(LOGERROR,"CDVDDemuxMPEGTS::Open - failed to load ffmpeg libraries");
		return false;
	}
...
void CDVDDemuxMPEGTS::Abort()
{
	g_urltimeout = 1;
}

void CDVDDemuxMPEGTS::SetSpeed(int iSpeed)
{
	if(!m_pFormatContext)
		return;
...
double CDVDDemuxMPEGTS::ConvertTimestamp(int64_t pts, int den, int num)
{
...
DemuxPacket* CDVDDemuxMPEGTS::Read()
{
	AVPacket pkt;
	...
	if (m_pFormatContext)
	{
		...
		else
		{
			...
			{
				CLog::Log(LOGERROR, "CDVDDemuxMPEGTS::Read() no valid packet");
				bReturnEmpty = true;
			}
			else
			{
...
bool CDVDDemuxMPEGTS::SeekTime(int time, bool backwords, double *startpts)
{
	if(time < 0)
		time = 0;
...
bool CDVDDemuxMPEGTS::SeekByte(__int64 pos)
{
	Lock();
...
void CDVDDemuxMPEGTS::UpdateCurrentPTS()
{
	m_iCurrentPts = DVD_NOPTS_VALUE;
	for(int i=0;i<MAX_STREAMS;i++)
	{
...
int CDVDDemuxMPEGTS::GetStreamLength()
{
	if (!m_pFormatContext)
		return 0;
...
CDemuxStream* CDVDDemuxMPEGTS::GetStream(int iStreamId)
{
	if (iStreamId < 0 || iStreamId >= MAX_STREAMS) return NULL;
	return m_streams[iStreamId];
}

int CDVDDemuxMPEGTS::GetNrOfStreams()
{
	int i = 0;
...
void CDVDDemuxMPEGTS::AddStream(int iId)
{
	...
	if (pStream)
	{
		...
		switch (pStream->codec->codec_type)
		{
			...
			{
				CDemuxStreamAudioMPEGTS* st = new CDemuxStreamAudioMPEGTS(this, pStream);
				m_streams[iId] = st;
			...
			{
				CDemuxStreamVideoMPEGTS* st = new CDemuxStreamVideoMPEGTS(this, pStream);
				m_streams[iId] = st;
			...
			{
				CDemuxStreamSubtitle* st = new CDemuxStreamSubtitleMPEGTS(this, pStream);
				m_streams[iId] = st;
				if(pStream->codec)
...
std::string CDVDDemuxMPEGTS::GetFileName()
{
...
int CDVDDemuxMPEGTS::GetChapterCount()
{
...
int CDVDDemuxMPEGTS::GetChapter()
{
	if(m_pInput && m_pInput->IsStreamType(DVDSTREAM_TYPE_DVD))
...
bool CDVDDemuxMPEGTS::SeekChapter(int chapter, double* startpts)
{
	if(chapter < 1)
		chapter = 1;
